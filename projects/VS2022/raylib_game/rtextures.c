...
void DrawTextureRecScaled(Texture2D texture, Rectangle source, Vector2 position, float scale, Color tint)
{
    // Adjust the destination rectangle based on the scale factor
    Rectangle dest = { position.x, position.y, fabsf(source.width) * scale, fabsf(source.height) * scale };
    Vector2 origin = { 0.0f, 0.0f };
